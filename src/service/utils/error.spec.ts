import { EKBackendError, ErrorCode } from "./error";

describe("EKBackendError", () => {
  const errorClassName = "EKBackendError";

  describe("isEKBackendError", () => {
    [
      { title: "object created via new", input: new EKBackendError(0, "message"), expected: true },
      {
        title: "object created via new with original error",
        input: new EKBackendError(1, "msg", new Error("original")),
        expected: true,
      },
      ...
      it(`should return ${testCase.expected} when input is ${testCase.title}`, () => {
        ...
        // act
        const actual = EKBackendError.isEKBackendError(testCase.input);
  ...
  describe("toString", () => {
    it("should return formatted string", () => {
      // arrange
      const target = new EKBackendError(1, "message", { error: "original" });
...
describe("ErrorCode", () => {
  describe("categorize()", () => {
    it("should return argument as-is if argument is instance of AssetEKBackendError", () => {
      // arrage
      const expected = new EKBackendError(ErrorCode.NOT_FOUND, "asset service error des");
